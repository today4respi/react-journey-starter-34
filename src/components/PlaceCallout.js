    ...
    <View style={styles.calloutContainer}>
      <View style={styles.callout}>
        {place.images && place.images.length > 0 ? (
          ...
          <Image 
            source={getDefaultImage(place.type)} 
            style={styles.placeImage}
            resizeMode="cover"
          />
        )}
        <Text style={styles.calloutTitle}>{place.name || place.nom_place}</Text>
          ...
          <View style={styles.categoryBadge}>
            <Text style={styles.categoryText}>
              {place.type === 'museum' ? 'Musée' :
               ...
               place.type ? place.type.charAt(0).toUpperCase() + place.type.slice(1) : 'Site'}
            </Text>
          </View>
          
          {place.average_rating && parseFloat(place.average_rating) > 0 && (
            <View style={styles.ratingContainer}>
              {Array(5).fill(0).map((_, i) => (
                <Star 
                  key={i}
                  size={12} 
                  color={i < Math.round(parseFloat(place.average_rating)) ? COLORS.warning : COLORS.gray_light}
                  fill={i < Math.round(parseFloat(place.average_rating)) ? COLORS.warning : 'transparent'}
                />
              ))}
              <Text style={styles.ratingText}>{parseFloat(place.average_rating).toFixed(1)}</Text>
            </View>
          )}
        </View>
