...
const Header = () => {
  ...
  return <>
          ...
          {isMobile && (
            <div className="flex items-center justify-between my-[-1%]">
              <div className="flex-1 flex justify-start">
                <Button variant="ghost" size="sm" onClick={() => setIsSidebarOpen(true)} className="p-3 hover:bg-slate-100 transition-colors rounded-lg">
              ...
              <div className="flex items-center justify-end flex-1 space-x-1 sm:space-x-2">
                <CartIcon />
                <Button variant="ghost" size="sm" onClick={handleProfileClick} className="p-3 hover:bg-slate-100 transition-colors rounded-lg flex items-center gap-2">
          ...
          {!isMobile && (
            <div className="flex items-center justify-between my-[-1%]">
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  <span className="text-slate-700 font-medium text-sm">Suivre une commande</span>
                </Button>
                <Button 
                  variant="ghost" 
                  size="sm" 
                  ...
                  className="p-3 hover:bg-slate-100 transition-colors rounded-lg flex items-center gap-2"
                >
                  <span className="text-slate-700 font-medium text-sm">Contactez nous</span>
                </Button>
                <Button variant="ghost" size="sm" onClick={handleProfileClick} className="p-3 hover:bg-slate-100 transition-colors rounded-lg flex items-center gap-2">
                  <User className="w-9 h-9 sm:w-10 sm:h-10 text-slate-700" />
      ...
      <MobileSidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} isLoggedIn={isAuthenticated} userEmail={user?.email || ''} onShowLogin={() => setShowLoginModal(true)} onShowAccount={() => {
      navigate('/account');
