...
export default function SignupScreen({ navigation }) {
  ...
  // Redirect to Map screen if already authenticated with Clerk
  useEffect(() => {
  ...
  const validateStep = (step) => {
    switch (step) {
      ...
      case 2:
        ...
        // Password validation (min 6 chars, contains letters and numbers)
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      if (createdSessionId) {
        ...
        // The useClerkIntegration hook will handle the rest
      }
    } catch (err) {
